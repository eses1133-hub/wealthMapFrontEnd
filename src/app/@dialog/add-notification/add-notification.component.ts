...
export class AddNotificationComponent {
  ...
  save() {
    ...
    if (!this.tagErrorMsg && !this.titleErrorMsg && !this.scheduledDateErrorMsg) {
      this.httpClientService.postApi(`https://backend-production-b9bbb.up.railway.app/api/notifications/save`, this.notificationList)
        .subscribe((save: any) => {
          if (save.code == 200) {
            console.log('通知儲存成功');
  ...
  edit() {
    ...
    if (!this.tagErrorMsg && !this.titleErrorMsg && !this.scheduledDateErrorMsg) {
      this.httpClientService.putApi(`https://backend-production-b9bbb.up.railway.app/api/notifications/update`, this.editNotificationList)
        .subscribe((save: any) => {
          if (save.code == 200) {
            console.log('更新成功');
            this.dialogRef.close();
