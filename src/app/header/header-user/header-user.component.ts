...
export class HeaderUserComponent {
  ...
  detail(pageId: number) {
    // 1. 呼叫已讀 API (POST)
    this.httpClientService.postApi(`https://backend-production-b9bbb.up.railway.app/api/notifications/read?userId=${this.userId}&notificationId=${pageId}`, {})
      .subscribe(() => {
  ...
  detailPersonal(logId: number) {
    ...
    this.httpClientService.patchApi(`https://backend-production-b9bbb.up.railway.app/api/notifications/${logId}/read`, {})
      .subscribe(() => {
  ...
  refreshUnreadCount() {
    ...
    //新增個人訊息未讀 by Carly
    this.httpClientService.getApi(`https://backend-production-b9bbb.up.railway.app/api/notifications/unread-count-new?userId=${this.userId}`)
      // this.httpClientService.getApi(`https://backend-production-b9bbb.up.railway.app/api/notifications/unread-count-new?userId=1`)
      .subscribe((res: any) => {
        if (res && res.code === 200) {
  ...
  fetchSystemNotifications() {
    this.httpClientService.getApi(`https://backend-production-b9bbb.up.railway.app/api/notifications/list-with-status?userId=${this.userId}`)
