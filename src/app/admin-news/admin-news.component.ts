...
export class AdminNewsComponent {
  ...
  onToggle(news: News) {
    ...
    this.httpClientService.postApi(`https://backend-production-b9bbb.up.railway.app/api/news/${news.id}/hide?hide=${nextStatus}`)
      .subscribe((res: any) => {
  ...
  fetchNews() {
    this.httpClientService.getApi(`https://backend-production-b9bbb.up.railway.app/api/news/admin/list`).subscribe({
      next: (data: any) => {
  ...
  fetchData() {
    ...
    const url = `https://backend-production-b9bbb.up.railway.app/api/news/admin/list?page=${this.p - 1}&size=${this.itemsPerPage}`;
